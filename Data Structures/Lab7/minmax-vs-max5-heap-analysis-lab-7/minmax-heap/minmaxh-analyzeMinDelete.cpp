...
void repeaterFunction(int size)
{
  ...
  MinMaxHeap* aHeap=nullptr;
	cout<<"Trial times for minmax min delete: for size "<<size<<" \n";
  double average=0.0;
  ...
  t=new Timer();
  aHeap=new MinMaxHeap(heapArray, size);
  t->start();
  aHeap->deleteMinElem();// adds an element to the end (constant time) then builds heap so same thing as just buildheap
