...
void repeaterFunction(int size)
{
  ...
  MinMaxHeap* aHeap=nullptr;
	cout<<"Trial times for minmax max delete: for size "<<size<<" \n";
  double average=0.0;
  ...
  t->start();
  aHeap->deleteMaxElem();// adds an element to the end (constant time) then builds heap so same thing as just buildheap
